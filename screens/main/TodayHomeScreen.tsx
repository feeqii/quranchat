...
type TodayHomeScreenNavigationProp = NativeStackNavigationProp<TodayStackParamList, 'TodayHomeScreen'>;
...
export const TodayHomeScreen: React.FC = () => {
  ...
  const handleViewResults = () => {
    ...
    if (!generatedReflection && !selectedVerse) {
      ...
      Alert.alert(
        'Content Missing',
        'Your spiritual reflection appears to be missing. Would you like to regenerate it?',
        [
  ...
  const renderPastReflectionCard = (reflection: any, dateFormatted: string) => {
    return (
      <View style={[styles.journeyCard, { backgroundColor: 'rgba(60, 140, 126, 0.05)' }]}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <Typography variant="h3" color={theme.colors.textPrimary} style={{ fontWeight: '700' }}>
            {dateFormatted} Reflection 🌙
          </Typography>
          <TouchableOpacity onPress={() => setSelectedDate(null)} style={{ padding: 4 }}>
            <Typography variant="body" color={theme.colors.textMuted}>✕</Typography>
          </TouchableOpacity>
        </View>
        
        <Typography variant="body" color={theme.colors.textSecondary} style={{ marginBottom: 8, fontSize: 12, fontWeight: '600' }}>
          MOOD
        </Typography>
        <Typography variant="body" color={theme.colors.textPrimary} style={{ marginBottom: 16 }}>
        ...
        {reflection.userInput && (
          <>
            ...
            <Typography variant="body" color={theme.colors.textPrimary} style={{ marginBottom: 16, lineHeight: 22 }}>
              {reflection.userInput}
            </Typography>
          </>
        )}
        ...
        {reflection.generatedReflection && (
          <>
            ...
            <Typography variant="body" color={theme.colors.textPrimary} style={{ marginBottom: 16, lineHeight: 22, fontStyle: 'italic' }}>
              {reflection.generatedReflection}
            </Typography>
          </>
        )}
        ...
        {reflection.selectedVerse && (
          <View style={{ backgroundColor: theme.colors.primarySoft, padding: 16, borderRadius: 12, marginBottom: 16 }}>
            <Typography variant="body" color={theme.colors.textPrimary} style={{ fontStyle: 'italic', lineHeight: 22 }}>
              "{reflection.selectedVerse.text}"
            </Typography>
            <Typography variant="caption" color={theme.colors.textMuted} style={{ marginTop: 8, textAlign: 'right' }}>
              — {reflection.selectedVerse.reference}
            </Typography>
          </View>
        )}
        
        <Typography variant="caption" color={theme.colors.primary} style={{ textAlign: 'center', fontWeight: '600' }}>
          ✅ Completed
        </Typography>
      </View>
    );
  ...
  const renderNoReflectionCard = (dateFormatted: string) => {
    return (
      <View style={[styles.journeyCard, { backgroundColor: 'rgba(139, 147, 150, 0.1)' }]}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <Typography variant="h3" color={theme.colors.textMuted} style={{ fontWeight: '700' }}>
            {dateFormatted}
          </Typography>
          <TouchableOpacity onPress={() => setSelectedDate(null)} style={{ padding: 4 }}>
            <Typography variant="body" color={theme.colors.textMuted}>✕</Typography>
        ...
        <Typography variant="body" color={theme.colors.textMuted} style={{ textAlign: 'center', marginBottom: 12 }}>
          No reflection recorded on this date.
        </Typography>
        
        <Typography variant="caption" color={theme.colors.textMuted} style={{ textAlign: 'center' }}>
          Try selecting a different date or return to today's journey.
        </Typography>
      </View>
    );
  };
  ...
  const renderJourneyCard = () => {
    ...
    if (!selectedDateData.isToday) {
      if (selectedDateData.reflection) {
        return renderPastReflectionCard(selectedDateData.reflection, selectedDateData.dateFormatted);
      } else {
          ...
          >
            ...
            >
              ...
              <View style={styles.journeyCompletedContent}>
                ...
                >
                  {moodDescription ? 
                    `Based on feeling ${moodDescription.toLowerCase()}, your personalized reflection and spiritual guidance are ready.` :
                    'Your personalized reflection and spiritual guidance are ready.'
                  }
                </Typography>
              </View>
      ...
      return (
        ...
        >
          <View style={styles.journeyCardContent}>
            ...
            >
              You're {progress}% through today's reflection. Let's finish creating your personalized spiritual content.
            </Typography>
      ...
      >
        <View style={styles.journeyCardContent}>
          ...
          <Typography
            ...
            style={styles.journeyDescription}
          >
            Take a moment to reflect on your day and receive personalized spiritual guidance.
          </Typography>
        </View>
